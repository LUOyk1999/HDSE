...
class TransConvLayer(nn.Module):
    def __init__(self, in_channels, out_channels, num_heads, use_weight=True, num_centroids=1024):
        super().__init__()
        self.Wk = nn.Linear(in_channels, out_channels * num_heads)
        self.Wq = nn.Linear(in_channels, out_channels * num_heads)
        self.Wv = nn.Linear(out_channels, out_channels * num_heads)
    ...
    def global_forward(self, x, nodes_to_community_tensor):
    
        d, h = self.out_channels, self.num_heads
        ...
        if self.training:
            x_idx = self.vq.update(x, nodes_to_community_tensor)
...
class TransConv(nn.Module):
    ...
    def forward(self, x, nodes_to_community_tensor, edge_index=None):
        layer_ = []
        ...
        for i, conv in enumerate(self.convs):
            ...
            x = conv.global_forward(x,nodes_to_community_tensor)
            if self.use_residual:
...
class Transformer(nn.Module):
    ...
    def forward(self, x, edge_index, nodes_to_community_tensor):
        x1 = self.trans_conv(x, nodes_to_community_tensor)
        if self.use_graph:
