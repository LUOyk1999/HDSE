...
def parse_method(args, c, d, device):
    if args.method == 'ours':
        model = Transformer(d, args.hidden_channels, c, trans_num_layers=args.ours_layers, alpha=args.alpha,
                    ...
                    use_graph=args.use_graph, aggregate=args.aggregate, num_centroids=args.num_centroids).to(device)
    else:
